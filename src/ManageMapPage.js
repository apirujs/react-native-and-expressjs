...
import {MenuButton,TestButton} from './custom.js';
import { Point, ClickEvents } from './class.js';
...
const ManageMapPage = () => {
  ...
  useEffect(()=>{
    console.log(String(renderCount));
  })
  ...
  const drawLine = (color, startPin, toPin) => {
    ...
    if (startPin.floor != toPin.floor) return;
    const ctx = canvas.current.getContext('2d');
  ...
  const LoadSetting = async () => {
    ...
    if (json.pins !== "" || json.pins != undefined)
    try{
      json = JSON.parse(json.pins);
    }    catch(err){
      console.log("could not parse loading Pins to JSON."); 
      json =[];
    }
        ...
        <View style={styles.PropStyles}>
          ...
          <View style={styles.propButtonLayer}>
            <View style={{ flexDirection: 'row' }}>
              ...
              <MenuButton style={styles.propButton} title='Save setting' onPress={() => SaveSetting(focusPin, temp)} />
              <TestButton title={"Test"} onPress={()=>console.log("Test")} style={styles.propButton}/>
            </View>
